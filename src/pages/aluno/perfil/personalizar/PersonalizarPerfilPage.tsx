...
import { ProfileIdentityCard, type SlotsCosmeticos } from '../../../../shared/ui/profile-identity-card';
import type { ItemInventario, TipoItemLoja } from '../../../../features/loja';

const ABAS: { id: TipoItemLoja; label: string; descricao: string }[] = [
  { id: 'ICONE_PERFIL', label: 'Ícones', descricao: 'Selecione um ícone que você já tem — ele aparece no seu avatar.' },
  { id: 'MOLDURA', label: 'Molduras', descricao: 'A moldura é aplicada ao redor do seu ícone de perfil.' },
  { id: 'AVATAR', label: 'Avatares', descricao: 'Personagem ilustrado exibido na sua página de perfil.' },
  { id: 'TITULO', label: 'Títulos', descricao: 'O título escolhido aparece abaixo do seu nome.' },
  { id: 'PLANO_FUNDO', label: 'Fundos', descricao: 'Aplicado ao topo da sua página de perfil.' },
];
...
export const PersonalizarPerfilPage = () => {
  ...
  const [inventario, setInventario] = useState<(ItemInventario & { inventarioId: string; equipado: boolean })[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchInventario = async () => {
      try {
        const response = await httpClient.get('/inventario/meuInventario');
        
        if (ativo && response.data?.dados) {
          const itensDoBackend = response.data.dados;
          setInventario(itensDoBackend);

          const equipadosReais: SlotsCosmeticos = {};
          
          // Correção de lint/build: tipagem forte em vez de 'any'
          itensDoBackend.forEach((item: ItemInventario & { equipado: boolean }) => {
            if (item.equipado) {
  ...
  const itensDaAbaAtual = inventario.filter((item) => item.tipo === abaAtiva);
    ...
    try {
      const promises = Object.values(stagedCosmetics).map(async (item) => {
        ...
        if (itemOriginal?.id !== item.id) {
          try {
            await httpClient.patch('/inventario/equipar', { itemLojaId: item.id });
          } catch (error) {
  ...
  const cursoLabel = [user.course, user.institution].filter(Boolean).join(' · ') || 'Curso não informado';
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="mx-auto flex max-w-5xl flex-col gap-6">
          ...
          <header>
            ...
            <p className="mt-1 text-sm font-semibold text-gray-500">Use seus itens para montar sua identidade</p>
          </header>
          ...
          <div className="grid items-start gap-6 lg:grid-cols-[300px_1fr]">
            ...
            <div className="sticky top-6 flex flex-col gap-4">
              <div className="rounded-2xl bg-white p-1 text-center shadow-sm">
                ...
                <div className="p-2">
                  <ProfileIdentityCard
                    identidade={{ nome: user.name, nickname, curso: cursoLabel }}
              ...
              <p className="text-center text-xs font-medium text-gray-400">
                Aqui aparecem só os itens que <b>você já tem</b>. Faltou algum? Vá à <button onClick={() => navigate('/aluno/loja')} className="font-bold text-[#14b8a6] hover:underline">Loja</button>.
              </p>
            </div>
            ...
            <div className="flex min-w-0 flex-col gap-6">
              ...
              <div className="flex flex-wrap gap-2">
                {ABAS.map((aba) => {
                  ...
                  const count = inventario.filter(i => i.tipo === aba.id).length;
                  return (
                    <button
                      key={aba.id}
                    ...
                    >
                      {aba.label}
                      <span className={`rounded-full px-2 py-0.5 text-[10px] ${isActive ? 'bg-white/20' : 'bg-gray-100 text-gray-600'}`}>
                        {count}
                      </span>
                    </button>
                  );
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex h-24 w-full items-center justify-center">
                          ...
                          {abaAtiva === 'ICONE_PERFIL' && (
                            ...
                            >
                              {item.previewImagemUrl || item.imagemUrl ? (
                                ...
                                <img src={item.previewImagemUrl || item.imagemUrl || undefined} alt={item.nome} className="h-full w-full object-contain drop-shadow-sm" />
                              ) : (
                          ...
                          {abaAtiva === 'AVATAR' && (
                            <img src={item.previewImagemUrl || item.imagemUrl || undefined} alt={item.nome} className="h-20 w-20 rounded-2xl border border-gray-200 bg-white object-contain p-1 shadow-sm" />
                          )}
                        </div>
                        
                        <div className="flex flex-col items-center">
                          <span className="text-sm font-black text-[#00214d] text-center leading-tight">{item.nome}</span>
                          {estaEquipado && <span className="mt-1 text-[10px] font-bold text-teal-600 uppercase tracking-wider">Equipado</span>}
                        </div>
                      </button>
                    );
                  })}
              ...
              {temAlteracoes && (
                <div className="mt-4 flex items-center justify-between rounded-xl border border-amber-200 bg-white p-4 shadow-sm">
                  ...
                  <div className="flex gap-3">
                    <button onClick={descartarAlteracoes} disabled={salvando} className="rounded-lg px-4 py-2 text-sm font-bold text-gray-500 hover:bg-gray-100 disabled:opacity-50">
                      Descartar
                    </button>
                    <button onClick={salvarAlteracoes} disabled={salvando} className="inline-flex items-center gap-2 rounded-lg bg-[#14b8a6] px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-[#0d9488] disabled:opacity-50">
