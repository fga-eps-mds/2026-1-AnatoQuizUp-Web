      ...
      <div className="w-full max-w-xl flex flex-col items-center gap-10">
        ...
        {isAuthenticated ? (
          <div className="w-full bg-[#0A1128] p-8 rounded-[2.5rem] shadow-2xl flex flex-col items-center gap-6 relative overflow-hidden">
            ...
            <div className="text-center">
              <p className="text-[#fffffe]/50 text-[10px] uppercase tracking-widest font-bold mb-1">
                {profileLabel}
              </p>
              <h2 className="text-[#fffffe] text-3xl font-bold">{name}</h2>
              {metadata && (
            ...
            {action && <div className="w-full flex flex-col gap-3 mt-4">{action}</div>}
          </div>
        ) : (
          <div className="w-full bg-[#0A1128] p-12 rounded-[2.5rem] shadow-2xl text-center flex flex-col gap-8">
            ...
            <Button onClick={onLogin}>Entrar agora</Button>
          </div>
        )}

        <div className="flex flex-col items-center gap-2 opacity-30">
          <p className="text-[#0A1128] text-[10px] font-bold uppercase tracking-tighter">
